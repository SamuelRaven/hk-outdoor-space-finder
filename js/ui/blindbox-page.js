...
function buildDiceNote(park) {
  ...
  if (park.parkType === '海濱長廊') {
    const p = [
      '骰子說：今天去聽聽海浪的聲音，把煩惱都沖走～',
      '命運指向海的方向，讓海風替你整理今天的思緒～',
      '海不會問你為什麼累，它只會靜靜陪著你～',
    ];
    return p[h % p.length];
  } else if (park.parkType === '市區公園') {
    const p = [
      '骰子說：即使身在城市中央，也值得為自己偷半小時的閒～',
      '命運提示：鋼筋森林裡的一小片綠，提醒你生活不只是趕路～',
      '鬧市中的喘息片刻，今天你值得這份寧靜～',
    ];
    return p[h % p.length];
  } else if (park.parkType === '郊野綠地') {
    const p = [
      '骰子說：把手機放下，讓眼睛被綠色好好治癒～',
      '命運把你推向山野，是時候重新連接大自然了～',
      '城市給不了的平靜，山林會加倍補給你～',
    ];
    return p[h % p.length];
  } else if (park.parkType === '主題園林') {
    const p = [
      '骰子說：美就藏在細節裡，今天放慢腳步去發現～',
      '命運安排你來賞景，別趕路，每一處都值得停留～',
